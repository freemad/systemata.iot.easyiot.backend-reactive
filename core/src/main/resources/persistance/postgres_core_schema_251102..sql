...
CREATE TABLE IF NOT EXISTS "device_data" (
    related_id          uuid not null, -- REFERENCES gate(id),
    ts                  timestamptz not null default now(),
    event_type          smallserial,
    event_source        smallserial,
    data                double precision not null,
    meta_data           jsonb null,
    CONSTRAINT pk_device_data PRIMARY KEY (related_id, event_type, ts)
);
SELECT create_hypertable('device_data', 'ts');
CREATE INDEX ON device_data (related_id, event_type, ts DESC);

-------------------------
-- RETENTION POLICY --
-------------------------
SELECT add_retention_policy('device_data', INTERVAL '180 days');
CREATE MATERIALIZED VIEW device_data_hourly
            WITH (timescaledb.continuous) AS
SELECT related_id, event_type, time_bucket('1 hour', ts) AS bucket, avg(data) AS avg_data
FROM device_data
GROUP BY related_id, event_type, bucket;
...
values ('12345678-1234-1234-1234-123456789ab0', 'admin', 'admin', now(), now(), 'Test Module 1.0',
        'TM1.0', 'the 1st test module of devices');

-- GATE_TYPE
insert into gate_type (id, created_by, last_modified_by, created_date, last_modified_date, name, is_input,
                       protocol_type, max_value, base_value, pin_count)
values ('12345678-1234-1234-1234-123456789ab0', 'admin', 'admin', now(), now(), 'bin. in', true, 1, 1, 0, 1);
insert into gate_type (id, created_by, last_modified_by, created_date, last_modified_date, name, is_input,
                       protocol_type, max_value, base_value, pin_count)
values ('12345678-1234-1234-1234-123456789ab1', 'admin', 'admin', now(), now(), 'bin. out', false, 1, 1, 0, 1);
